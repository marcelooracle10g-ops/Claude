...
const insertSampleOrders = db.transaction(() => {
  ...
  for (const month of months) {
    for (let repId = 2; repId <= 5; repId++) {
      const rep = db.prepare('SELECT * FROM users WHERE id = ?').get(repId);
      ...
      for (let i = 0; i < numOrders; i++) {
        ...
        `).run(rep.brand_id, repId, customerId, total, total, status);
        ...
        const commissionStatuses = ['pending', 'scheduled', 'paid'];
        ...
        `).run(orderId, repId, total, rep.commission_rate, commissionValue, commissionStatus, paymentDate);
      }
    }
  }
});
...
console.log('Admin login: admin@cosmeticos.com / admin123');
console.log('Rep login: carlos@cosmeticos.com / rep123');
